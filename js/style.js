...
function graphic(s,e){
    google.charts.load('current', {'packages':['corechart']});
      ...
      async function drawChart() {
        ...
        data.addColumn('string', 'fornisseur');
        ...
        // Set chart options
        var options = {'title':'Fournisseur',
                       'width':340,
                       'height':270,
                      'colors':['#6a89cc','#9980FA','#60a3bc']};
...
function new_row(){
    ...
    let upd = function(){ht.innerText=(prix[p.children[0].value]*q.children[0].value).toString();ttc.innerText=((prix[p.children[0].value]*(1+tva)*q.children[0].value).toFixed(2)).toString();
        ...
        id("rg_nm").value = id("enter_name").value;id("rg_mt").value = parseFloat(id("tot_ttc").innerText).toFixed(2);
    };
...
function fnew_row(){
    ...
    let upd = function(){ht.innerText=(prix[p.children[0].value]*q.children[0].value).toString();ttc.innerText=((prix[p.children[0].value]*(1+tva)*q.children[0].value).toFixed(2)).toString();
        ...
        id("rg_nm").value = id("fenter_name").value;id("rg_mt").value = id("ftot_ttc").innerText;
    };
    p.addEventListener("change",function(){upd()});
    q.addEventListener("change", function(){upd()});
...
window.onload = function() {
    ...
    id("sdas").addEventListener("click", function(){
        if(corf==0){
            ...
            eel.add_facture([id("rg_nm").value,parseFloat(id("rg_mt").value).toFixed(2),id("rg_es").value],function(){let s=[];for(let i=0; i<id("tb_prod").children.length;i++){s[i]=[id("tb_prod").children[i].children[0].children[0].value,id("tb_prod").children[i].children[1].children[0].value,parseFloat(id("tb_prod").children[i].children[3].innerText).toFixed(2)]}return s;}());
        ...
        if(corf==1){
            eel.fadd_facture([id("rg_nm").value,parseFloat(id("rg_mt").value).toFixed(2),id("rg_es").value],function(){let s=[];for(let i=0; i<id("ftb_prod").children.length;i++){s[i]=[id("ftb_prod").children[i].children[0].children[0].value,id("ftb_prod").children[i].children[1].children[0].value,parseInt(id("ftb_prod").children[i].children[3].innerText)]}return s;}());
            updateall();id("ass").classList.remove("active_regelement");
            for(let i=0;i<id("chit").children.length;i++){
    ...
    id("chite").addEventListener("keydown",function(event){if(event.key=="Enter"){
        let somme=0;for(let i=0;i<id("chite").children.length;i++){ if(id("chite").children[i].children[1].children[0].value ==""){alert('remplir toutes les dates'); return;} somme+=parseFloat(id("chite").children[i].children[2].children[0].value)}; if(parseFloat(somme).toFixed(2) != parseFloat(lastch.children[5].innerText).toFixed(2)) {alert("somme incomplete "+somme.toString()+" != "+lastch.children[5].innerText);return;};
    ...
    document.addEventListener("keydown",function(){
        ...
        if(event.key=="Shift"){if(sel[0]=="chqs" && sel[1].children[6].innerText!="Parted" && sel[1].children[6].innerText!="Valid"){updateall();id("part").style="display:block;"}}
    });
